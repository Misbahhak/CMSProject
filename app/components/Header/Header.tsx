...
const Header = () => {
    ...
    const userAvatar = session?.user ? (
        <div className="w-10 h-10 rounded-full overflow-hidden">
            <Image src={session.user.image || "/default-avatar.png"} alt={session.user.name || "User"} width={40} height={40} className="scale-animation img" />
        </div>
    ) : (
        ...
        <header className="bg-white border-gray-200 dark:bg-gray-900">
            <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
                <Link href="/" className="flex items-center space-x-3 rtl:space-x-reverse">
                    <Image src="/hotellogo.svg" width={50} height={50} alt="Hotelzz Logo" />
                    <span className="self-center text-2xl font-semibold whitespace-nowrap dark:text-white">Hotelzz</span>
                </Link>
                <div className="flex items-center space-x-3 md:order-2 rtl:space-x-reverse">
                    <div className="md:self-center">
                        {darkTheme ? (
                            <MdOutlineLightMode className="cursor-pointer w-6 h-6 md:w-6 md:h-6" onClick={themeToggleHandler} />
                        ) : (
                            <MdDarkMode className="cursor-pointer w-6 h-6 md:w-6 md:h-6" onClick={themeToggleHandler} />
                        )}
                    </div>
                    <div className="relative">
                        <button type="button" className="flex text-sm bg-gray-800 rounded-full md:me-0 focus:ring-4 focus:ring-gray-300 dark:focus:ring-gray-600" id="user-menu-button" aria-expanded="false" onClick={() => setDropdownOpen(!dropdownOpen)}>
                        ...
                        {dropdownOpen && (
                            <div className="z-50 absolute right-0 my-4 text-base list-none bg-white divide-y divide-gray-100 rounded-lg shadow dark:bg-gray-700 dark:divide-gray-600" id="user-dropdown">
                                <div className="px-4 py-3">
                                    <span className="block text-sm text-gray-900 dark:text-white">{session?.user?.name || "Guest"}</span>
                                    <span className="block text-sm text-gray-500 truncate dark:text-gray-400">{session?.user?.email || "guest@example.com"}</span>
                                </div>
                ...
                <div className={`items-center justify-between w-full md:flex md:w-auto md:order-1 ${menuOpen ? 'block' : 'hidden'}`} id="navbar-user">
